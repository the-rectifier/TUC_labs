...
then calls the add_rule() function.
    ...
    5.) For the remaining operations the script is just a wrapper around the iptables commands providing the correct arguments and input/outpur redirection to save/load
        to and from specific files.

    Note: The IPAddresses.txt comes preloaded with the resolved ips of the domainNames.txt file
    Note 2: This tool is useless against in-video youtube ads because youtube does not use domains for these ads, but rather one hostname for both video and ads.
